...
StatusType courseManager::RemoveCourse(int courseID)
{
    ...
    for (int i = 0; i < number_of_classes; i++)
    {
        ...
        if (class_to_remove)
        {
            ...
            {
                // printCourseNode2(class_to_remove);
        ...
        course_pointer->getValue()->setNullPointer(i);
    }
    return (StatusType)this->getCourses()->remove(course_pointer->getValue());
}
...
StatusType courseManager::WatchClass(int courseID, int classID, int time)
{
    ...
    if (curr_time > 0)
    {
        this->getClasses()->remove((ptr->getValue()));
    }
    else
    {
        deleteNode(ptr);
    ...
    else
    {
        ...
        // std::cout << this->getClasses()->getRoot()->getRank() << std::endl;
        return res;
    }
}
...
StatusType courseManager::GetIthWatchedClass(int index, int *courseID, int *classID)
{
    if (index <= 0)
    {
        return INVALID_INPUT;
    ...
    if (!this->getClasses())
        return FAILURE;
    if (!this->getClasses()->getRoot())
        return FAILURE;
    int avl_tree_size = this->getClasses()->getRoot()->getRank();
    // if (index == 35)
    std::cout << this->getClasses()->getRoot()->getRank() << std::endl;
    if (avl_tree_size < index)
    {
        return FAILURE;
    }
    // this->getClasses()->inOrder(this->getClasses()->getRoot(), printCourseNode2);

    avlNode<classNode> *wanted_class = select(this->getClasses()->getRoot(), avl_tree_size + 1 - index);
    if (!wanted_class)
        return FAILURE;
    else
    {
        *courseID = wanted_class->getValue()->getCourseId();
        *classID = wanted_class->getValue()->getKey();
    }
    return SUCCESS;
}
